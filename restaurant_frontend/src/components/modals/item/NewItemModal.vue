...
export default defineComponent({
  ...
  setup() {
    ...
    const newItemModalRef = ref<null | HTMLElement>(null);
    const loading = ref<boolean>(false);

    const itemData = ref<NewAddressData>({
      name: "",
      parent_id: null,
      price:null,
    });
    ...
    const rules = ref({
      ...
      parent: [
        {
          required: true,
          message: "Please choose item parent",
          trigger: "blur",
        },
      ],
    });
    ...
    const submit = () => {
      ...
      formRef.value.validate((valid) => {
        if (valid) {
          ...
          axios.post(process.env.VUE_APP_API_URL + '/new-item',itemData.value)
              .then(function (response) {
                hideModal(newItemModalRef.value);
                Swal.fire({
